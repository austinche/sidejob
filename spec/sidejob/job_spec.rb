...
describe SideJob::Job do
  ...
  describe '#log' do
    ...
    it 'logs to SideJob.log if logger not set' do
      expect(SideJob).to receive(:log).with @entry
      @job.log(@entry)
    end

    it 'logs to job logger if set' do
      expect(SideJob).not_to receive(:log).with @entry
      @job.logger = Class.new
      expect(@job.logger).to receive(:log).with @entry
      @job.log(@entry)
    end
  end
  ...
  describe '#group_port_logs' do
    ...
    it 'passes through logs not generated by port read/write' do
      @job.group_port_logs do
        @job.log({'test' => 123})
        expect(SideJob.logs).to eq [{'timestamp' => SideJob.timestamp, 'test' => 123}]
      end
    end
